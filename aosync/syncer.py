...
class AppOpticsSyncer:
    ...
    def __init__(self, token):
        self.api = AppOpticsAPI(token)

    def sync(self, appoptics_dashboard: Dashboard):
        ...
        if existing_dashboard:
            updated_dashboard = self.api.update_dashboard(appoptics_dashboard.id, asdict(appoptics_dashboard))
        else:
            updated_dashboard = self.api.create_dashboard(asdict(appoptics_dashboard))
